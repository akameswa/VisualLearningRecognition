...
class DiffusionModel(nn.Module):
    ...
    def predict_denoised_at_prev_timestep(self, x, t: int):
        ...
        mean, var, _ = self.get_posterior_parameters(x_0, x, t)
        pred_img = mean + (torch.all(t > 0) * torch.randn_like(mean) * var.sqrt())
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################
        return pred_img, x_0
    ...
    def ddim_step(self, batch, device, tau_i, tau_isub1, img, model_predictions, alphas_cumprod, eta):
        ...
        epsilon_t, x_0 = model_predictions(img, t)
        ...
        alpha_prev = extract(alphas_cumprod, t_prev, img.shape)
        
        # Step 3: Compute \sigma_{\tau_{i}}
        beta_prev = extract(self.betas, t_prev, img.shape)
        sigma = eta * torch.sqrt(((1 - alpha_prev) / (1 - alpha_t)) * beta_prev)
        ...
        mu = torch.sqrt(alpha_prev) * x_0 + torch.sqrt(1 - alpha_prev - sigma**2) * epsilon_t
        z = torch.randn_like(img) if tau_isub1 > 0 else 0
        img = mu + sigma * z
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################
