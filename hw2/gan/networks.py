...
class DownSampleConv2D(torch.jit.ScriptModule):
    ...
    def forward(self, x):
        ...
        b, c, h, w = x.shape
        x = x.reshape(b, self.downscale_ratio*self.downscale_ratio, -1, h, w).permute(1,0,2,3,4)
        x = x.mean(0)
        return self.conv(x)
        ##################################################################
        #                          END OF YOUR CODE                      #
...
class ResBlockUp(torch.jit.ScriptModule):
    ...
    def __init__(self, input_channels, kernel_size=3, n_filters=128):
        ...
        self.layers = nn.Sequential(
            ...
            nn.ReLU(),
            UpSampleConv2D(n_filters, n_filters=n_filters, kernel_size=(3, 3), padding=(1, 1)),
        )
...
class ResBlockDown(torch.jit.ScriptModule):
    ...
    def __init__(self, input_channels, kernel_size=3, n_filters=128):
        ...
        self.layers = nn.Sequential(
            ...
            nn.ReLU(),
            DownSampleConv2D(n_filters, n_filters=n_filters, kernel_size=(3, 3), padding=(1, 1))
        )
        self.downsample_residual = DownSampleConv2D(input_channels, n_filters=n_filters, kernel_size=(1, 1))
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################
...
class Generator(torch.jit.ScriptModule):
    ...
    def forward_given_samples(self, z):
        ...
        ##################################################################
        z = self.dense(z)
        z = z.view(-1, 128, self.starting_image_size, self.starting_image_size)
        z = self.layers(z)
        return z
        ##################################################################
        #                          END OF YOUR CODE                      #
